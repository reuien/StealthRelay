...
import Item.Custom;
import PanelPackage.OwnerMainView;
...
public class mainframe_owner extends JFrame {
	 ...
	 * @throws IOException
	 */
	public mainframe_owner() throws IOException {
		initialize();
	}
	...
	public void initialize() throws IOException {
		...
		frame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE); // 修改部分: 设置默认关闭操作为不执行任何操作
		...
		JLabel lblNewLabel = new JLabel("数 据 拥 有 者 登 录");
		...
		login_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (user.equals("") || user == null) {
					...
					boolean matchFound = sqlsearch.Owner_Login(user, password);
					if (matchFound) {
						result = "登录成功！";
						new JFrame();
						JOptionPane.showMessageDialog(frame, result);
						try {
							new OwnerMainView(user, serverController);
						} catch (Exception e1) {
							e1.printStackTrace();
						}
						frame.dispose();
		...
		regist_button.setBounds(303, 228, 76, 23);
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					if (name.isEmpty() || password.isEmpty() || number.isEmpty()) {
						...
						if (number.length() == 4) {
							...
							if (!sql.NumberIsExisted(number, "拥有者")) {
								if (!sql.NameIsExisted(name, "拥有者")) {
									Custom custom = new Custom(name, number, password, "拥有者");
