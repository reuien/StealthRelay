...
public class TimeUtil {
    ...
    public static Precision[] getHigherPrecisions(Precision precision) {
        ...
        // 获取比 selectedOrdinal 大的所有精度
        Precision[] higherPrecisions = Arrays.stream(precisions)
                .filter(p -> p.ordinal() > selectedOrdinal)
                .toArray(Precision[]::new);

        // 如果过滤后的数组为空，则添加最大精度
        if (higherPrecisions.length == 0) {
            higherPrecisions = new Precision[]{precisions[precisions.length - 1]};
        }

        return higherPrecisions;
    }
